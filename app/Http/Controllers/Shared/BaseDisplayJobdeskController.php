...
/**
 * Controller dasar untuk Display Job Desk.
 *
 * Admin\DisplayJobdeskController, KPH\DisplayJobdeskController, dan
 * operator_display.php masing-masing meng-extends/menggunakan class ini.
 * Perbedaan antar role hanya pada routePrefix() yang menentukan nama route.
 */
abstract class BaseDisplayJobdeskController extends Controller
{
    /**
     * Prefix route untuk role ini, mis. 'admin', 'kph', atau 'operator-display'.
     */
    abstract protected function routePrefix(): string;

    public function index()
    {
        return redirect()->route("{$this->routePrefix()}.display-jobdesk.manage");
    }
    ...
    public function manage()
    {
        ...
        return view('pages.display.manage', [
            ...
            'tvRoute'             => route("{$this->routePrefix()}.display-jobdesk.tv"),
            'stateRoute'          => route("{$this->routePrefix()}.display-jobdesk.state"),
            'settingUpdateRoute'  => route("{$this->routePrefix()}.display-jobdesk.settings.update"),
            'resetRoute'          => route("{$this->routePrefix()}.display-jobdesk.reset"),
            'setting'             => $setting,
            'logs'                => $logs,
            'summary'             => $summary,
        ]);
    ...
    public function tv()
    {
        ...
        return view('pages.display.jobdesk', [
            'scanRoute'              => route("{$this->routePrefix()}.display-jobdesk.scan"),
            'stateRoute'             => route("{$this->routePrefix()}.display-jobdesk.state"),
            'backRoute'              => route("{$this->routePrefix()}.display-jobdesk.manage"),
            'pageTitle'              => 'Display Job Desk Harian',
    ...
    public function updateSettings(UpdateDisplaySettingsRequest $request)
    {
        $setting = DisplayJobdeskSetting::active();
        $setting->update([
            'display_duration_seconds' => $request->display_duration_seconds,
